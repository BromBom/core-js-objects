...
function mergeObjects(objects) {
  ...
  objects.forEach((obj) => {
    ...
    for (let i = 0; i < arrEntries.length; i += 1) {
      const [key, value] = arrEntries[i];
      if (Object.hasOwn(mergObj, key)) {
        mergObj[key] += value;
      }
    }
    mergObj = Object.assign(obj, mergObj);
  });
  return mergObj;
}
...
function compareObjects(obj1, obj2) {
  const arr1Entries = Object.keys(obj1);
  const arr2Entries = Object.keys(obj2);
  if (arr1Entries.length !== arr2Entries.length) return false;
  for (let i = 0; i < arr1Entries.length; i += 1) {
    if (obj1[arr1Entries[i]] !== obj2[arr2Entries[i]]) return false;
  }
  return true;
}
...
function makeWord(lettersObject) {
  const arrKeysObject = Object.entries(lettersObject);
  const arrWordString = [];
  for (let i = 0; i < arrKeysObject.length; i += 1) {
    const [char, arrValue] = arrKeysObject[i];
    arrValue.forEach((position) => {
      arrWordString[position] = char;
    });
  }
  return arrWordString.join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    switch (queue[i]) {
      case 25:
        objSellerPocket['25'] += 1;
        break;
      case 50:
        if (objSellerPocket['25'] >= 1) {
          objSellerPocket['50'] += 1;
          objSellerPocket['25'] -= 1;
        } else return false;
        break;
      case 100:
        if (objSellerPocket['50'] >= 1 && objSellerPocket['25'] >= 1) {
          objSellerPocket['100'] += 1;
          objSellerPocket['50'] -= 1;
          objSellerPocket['25'] -= 1;
        } else if (objSellerPocket['25'] >= 3) {
          objSellerPocket['100'] += 1;
          objSellerPocket['25'] -= 3;
        } else return false;
        break;
      default:
    }
...
function fromJSON(proto, json) {
  const objProps = JSON.parse(json);
  const props = Object.values(objProps);
  return new proto.constructor(...props);
}
